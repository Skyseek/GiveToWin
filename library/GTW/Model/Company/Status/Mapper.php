...
/**
 * Company Status Mapper
 *
 * @package    Givetowin
 * @copyright  Copyright (c) 2011, Give to Win, Inc
 * @author     Sean Thayne <sean@skyseek.com
 */
class GTW_Model_Company_Status_Mapper extends Skyseek_Model_Mapper 
{

	protected $_tableName = 'company_status';
	...
	/**
	 * @return GTW_Model_Company_Status_Collection
	 */
	public function getCollection(Skyseek_Model_Entity_Collection_Request $request = null) 
	{
		...
		$collection = new GTW_Model_Company_Status_Collection();

		foreach ($select->query()->fetchAll() as $data) {
			$collection->addItem($this->createCompanyStatusEntity($data, false));
		}
	...
	public function getPaginator(Skyseek_Model_Entity_Collection_Request $request) 
	{
		...
		$paginator->setFilter(new GTW_Model_Company_Status_Collection());
	...
	public function getStatus($id, $lazyLoad=true, $useIdentityMap=true) 
	{
		...
		$entity = $this->createCompanyStatusEntity($result, $lazyLoad);
	 ...
	 */
	private function createCompanyStatusEntity($data, $lazyLoad) 
	{

		$entity = new GTW_Model_Company_Status(array(
			'id'			=> $data['id'],
			'status'		=> $data['status'],
