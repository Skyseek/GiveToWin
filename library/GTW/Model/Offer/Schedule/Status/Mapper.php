...
/**
 * Offer Schedule Status Mapper
 *
 * @package    Givetowin
 * @copyright  Copyright (c) 2011, Give to Win, Inc
 * @author     Sean Thayne <sean@skyseek.com
 */
class GTW_Model_Offer_Schedule_Status_Mapper extends Skyseek_Model_Mapper 
{

	protected $_tableName = 'offer_schedule_status';
	...
	/**
	 * @return GTW_Model_Offer_Schedule_Status_Collection
	 */
	public function getCollection(Skyseek_Model_Entity_Collection_Request $request = null) 
	{
		...
		$collection = new GTW_Model_Offer_Schedule_Status_Collection();

		foreach ($select->query()->fetchAll() as $data) {
			$collection->addItem($this->createOfferStatusEntity($data, false));
		}
	...
	public function getPaginator(Skyseek_Model_Entity_Collection_Request $request) 
	{
		...
		$paginator->setFilter(new GTW_Model_Offer_Schedule_Status_Collection());
	...
	public function getStatus($id, $lazyLoad=true, $useIdentityMap=true) 
	{
		...
		$entity = $this->createOfferStatusEntity($result, $lazyLoad);
	 ...
	 */
	private function createOfferStatusEntity($data, $lazyLoad) 
	{

		$entity = new GTW_Model_Offer_Schedule_Status(array(
			'id'			=> $data['id'],
			'status'		=> $data['status'],
