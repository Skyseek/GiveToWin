/*
	...
	mootools 1.3.0 compatible
	updated 2010.10.20
		(c) 2010 Marcelo Origoni <http://marcelo.origoni.com.ar>
	updated 2010.09.21
...
//			Vimeo options
			...
			if(_options){
				Object.append(options, _options);
			}
			...
			if (Browser.firefox || Browser.firefox2) {	// Fixes Firefox 2 and Camino 1.6 incompatibility with opacity + flash
				foxfix = true;
				options.overlayOpacity = 1;
				overlay.className = 'mbOverlayFF';
			}

			if (Browser.ie && !(Browser.ie6 || Browser.ie7 || Browser.ie8)) {	// Fixes IE 6 and earlier incompatibilities with CSS position: fixed;
				iefix = true;
			...
			var resizeOpts = options.resizeTransition ? {transition: options.resizeTransition,duration: options.resizeDuration, onComplete: nextEffect} : {};			
			fx.resize = new Fx.Morph(center, resizeOpts);
	...
	function setup(open) {
		...
		if (Browser.firefox || Browser.firefox2 || Browser.firefox3) {
...
//			if (options.overflow) image.setStyles({overflow: options.overflow});
		...
		image.setStyles({width: mediaWidth, height: mediaHeight,opacity: '1',visibility:'visible'});
